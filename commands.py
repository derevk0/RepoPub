...
# -------------------
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run pre/post checks or command execution on network devices.")
    ...
    parser.add_argument("--mode", choices=["diff", "exec"], required=True,
                        help="Select mode: 'diff' for pre/post/diff comparison, 'exec' for command execution only")
...
# -------------------
def process_device_diff(device, commands, username, password, output_dir):
    """Run pre/post checks and generate diffs."""
    host = device["host"]
...
def process_device_exec(device, commands, username, password, output_dir):
    """Run commands only (no pre/post/diff)."""
    host = device["host"]
    device_type = device["device_type"]
    log_buf = StringIO()
    log_buf.write(f"\n[🔧 Device: {host}]\n")
...
def main():
    ...
    output_dir = args.output_dir or f"{args.change_number}"
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        for device in devices:
            if args.mode == "diff":
                ...
                futures.append(executor.submit(process_device_exec, device, commands, args.username, args.password, output_dir))
