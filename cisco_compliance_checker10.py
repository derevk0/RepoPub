...
def get_running_config_from_device(
    host: str, username: str, password: str, device_type: str, secret: Optional[str], outdir: str, save_config: bool
) -> str:
    device = {
    ...
    try:
        with ConnectHandler(**device) as conn:
            ...
            if save_config:
                ...
                logging.debug(f"Skipping saving running-config for {host}")
...
# -------- Summary HTML --------
def write_summary_html(all_device_results: Dict[str, Dict[str, str]], outfile: str) -> str:
    sections = set()
    for device, results in all_device_results.items():
        sections.update(results.keys())
    sections = sorted(list(sections))

    final_file = outfile
    with open(final_file, "w") as f:
        f.write("<html><head><title>Compliance Summary Report</title>\n")
        f.write("<style>\n")
        f.write("""
            body { font-family: Arial, sans-serif; margin: 20px; background: #ffffff; color: #333; }
            ...
            .status-fail { color: red; font-weight: bold; }
            .status-error { color: orange; font-weight: bold; }
            .report-date { font-size: 16px; color: #666; }  /* slightly bigger date font */
        """)
        f.write("</style></head><body>\n")
        f.write(f"<h1>Compliance Summary Report<br><span class='report-date'>{datetime.now().strftime('%A, %d %B %Y %I:%M %p')}</span></h1>\n")
        f.write("<table><tr><th>Device</th>")
        for sec in sections:
            f.write(f"<th>{sec}</th>")
        f.write("</tr>\n")

        for device, results in all_device_results.items():
            f.write(f"<tr><td>{device}</td>")
            for sec in sections:
                status = results.get(sec, "ERROR")
                css_class = "status-pass" if status == "PASS" else ("status-fail" if status == "FAIL" else "status-error")
                f.write(f"<td class='{css_class}'>{status}</td>")
            f.write("</tr>\n")

        f.write("</table></body></html>\n")

    logging.debug(f"Summary HTML written to {final_file}")
    return final_file
...
def main() -> int:
    ...
    parser.add_argument("--no-save-config", action="store_true", help="Do not save running-config files in output directory")
    parser.add_argument("--summary-html", help="Generate summary HTML file for all devices")
    ...
    all_device_results: Dict[str, Dict[str, str]] = {}
    ...
    for device in devices:
        if args.config_file:
            ...
            running_config = get_running_config_from_device(
                device, args.username, args.password, args.device_type, args.secret, args.outdir, not args.no_save_config
            )
            source_info = device
            suffix = device

        if running_config is None:
            # If device unreachable, mark all sections as ERROR
            section_status = {sec: "ERROR" for sec in policy.keys()}
            all_device_results[device] = section_status
            continue

        checker = ComplianceChecker(running_config, policy)
        results = checker.run_all()

        # Collect section status for summary
        section_status = {}
        grouped = group_results(results)
        for section, rows in grouped.items():
            statuses = [r["status"] for r in rows]
            if "FAIL" in statuses:
                section_status[section] = "FAIL"
            elif all(s == "PASS" for s in statuses):
                section_status[section] = "PASS"
            else:
                section_status[section] = "ERROR"
        all_device_results[device] = section_status
    ...
    if args.summary_html:
        summary_path = os.path.join(args.outdir, args.summary_html)
        write_summary_html(all_device_results, summary_path)
        print(f"Summary HTML written to {summary_path}")
