...
3) Or optionally reads a local config file (--config-file).
4) Calls another package/library to do compliance checks -> uses `ciscoconfparse` to parse and evaluate the config.
   (pip install ciscoconfparse)
5) Writes results to CSV or HTML (select via --output csv|html).
...
Example usage:
    ...
    # Run against a saved running-config file
    python cisco_compliance_checker.py \
        --config-file running-config.txt --output csv --outfile results.csv

Optional policy file (YAML) to customize checks.
See sample policy.yaml included below in this file header.
"""

"""
policy_name: "Sample Global Baseline"
logging:
...
def get_running_config(args) -> str:
    if args.config_file:  # New option: read from file
...
def main() -> int:
    ...
    parser.add_argument("--host", help="Device hostname/IP")
    parser.add_argument("--username")
    parser.add_argument("--password")
    parser.add_argument("--secret", default="")
    parser.add_argument("--device-type", default="cisco_ios")
    parser.add_argument("--config-file", help="Use local running-config file instead of device connection")
