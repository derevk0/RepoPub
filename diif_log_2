...
def parse_arguments():
    ...
    parser.add_argument("--username", help="Username to login", required=True)
    parser.add_argument("--password", help="Password to login (optional, will prompt if not supplied)")
...
def connect_and_execute(device, command, creds, log_buf):
    host = device['host']
    try:
        log_buf.write(f"[{host}] Connecting...\n")
...
def generate_diff_html(pre_lines, post_lines, host, command, output_dir, log_buf):
    ...
    with open(html_path, "w") as f:
        ...
        f.write(html)
    log_buf.write(f"[{host}] 🧾 Diff generated: {html_path}\n")
    logging.info(f"{host} - Diff saved: {html_path}")

def process_device_command(device, command, output_dir, creds):
    host = device['host']
    log_buf = StringIO()
    log_buf.write(f"\n=== Device: {host} ===\n")
    log_buf.write(f"Command: '{command}'\n")

    output = connect_and_execute(device, command, creds, log_buf)
    if output is None or "[ERROR]" in output:
        log_buf.write(f"[{host}] Skipping due to error.\n")
    ...
    if not os.path.exists(pre_path):
        ...
        log_buf.write(f"[{host}] PRE-test saved to: {pre_path}\n")
        logging.info(f"{host} - Pre-test saved")
    else:
        save_output(post_path, output)
        log_buf.write(f"[{host}] POST-test saved to: {post_path}\n")
        ...
        if pre_lines and post_lines:
            log_buf.write(f"[{host}] Generating diff...\n")
...
def main():
    ...
    password = args.password or getpass("Enter device password: ")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for device in devices:
            for command in commands:
                futures.append(executor.submit(process_device_command, device, command, output_dir, creds))
